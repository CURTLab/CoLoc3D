...
view(2)
axis image
hh=text(2*min(pos1(:,1)),4*min(pos1(:,2)), ['Cluster ',num2str(i),'/',num2str(M2)],'color','k');
...
subplot('position',pozycja1)
grid on
plot3(hullo(:,1),hullo(:,2),hullo(:,3),'.b','markersize',4)
hold on
if exist('C000','var')==1
...
subplot('position',pozycja1)
grid on
plot3(hull2(:,1),hull2(:,2),hull2(:,3),'.r','markersize',2)
hold on
if exist('C2','var')==1
trisurf(C2,hull2(:,1),hull2(:,2),hull2(:,3),'FaceColor','r','FaceAlpha',0.4)
 ...
 hold on
 hte=[hull2',hull2(1,:)']';
 plot3(hte(:,1),hte(:,2),hte(:,3),'-or','markersize',3)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
  ...
  plot3(hull1(:,1),hull1(:,2),hull1(:,3),'.b','markersize',4)
  hold on
  hte=[hull1',hull1(1,:)']';
  plot3(hte(:,1),hte(:,2),hte(:,3),'-ob','markersize',3)
  end
    ...
    if isempty(ps)==0 && size(ps,1)>3
        ...
        subplot('position',pozycja1)
        grid on
        plot3(ps(:,1),ps(:,2),ps(:,3),'ow','markerfacecolor','r','markersize',4)
        hold on
        if exist('C3','var')==1
        trisurf(C3,ps(:,1),ps(:,2),ps(:,3),'FaceColor','w','FaceAlpha',0.7)
        end
        subplot('position',pozycja2)
   ...
   elseif isempty(ps)==0 && size(ps,1)<=3  
        ...
        figure(200)
        subplot('position',pozycja1)
        plot3(ps(:,1),ps(:,2),ps(:,3),'ow','markerfacecolor','m','markersize',5)
        hold on
        if exist('C3','var')==1
        trisurf(C3,ps(:,1),ps(:,2),ps(:,3),'FaceColor','w','FaceAlpha',1)
        end
        hold on
        hte=[ps',ps(1,:)']';
        plot3(hte(:,1),hte(:,2),hte(:,3),'-ow','markersize',3,'markerfacecolor','m')
        subplot('position',pozycja2)
        bar(i,(v3/(v2+v1-v3))*100,'r')      
  elseif isempty(ps)
   ...
   figure(200)
        subplot('position',pozycja1)
        plot3(hull2(:,1),hull2(:,2),hull2(:,3),'ow','markerfacecolor','r','markersize',4)
        hold on
        if exist('C2','var')==1
        trisurf(C2,poly2(:,1),poly2(:,2),poly2(:,3),'FaceColor','w','FaceAlpha',0.4)
        end
        subplot('position',pozycja2)
 ...
 if stat==0
  ...
  D=pdist2(centroid2(i,:),kcentroid1)';
for j=1:10
    
      e1=find(D==min(D),1,'first');
      D(e1,:)=10000000;
      e2=find(klaster1(:,2)==e1);
      hull1=pos1(e2,:);
   ...
   if numel(e2)>3
        ...
        vol1=v1;
         ...
         zp=hull1;
         for nk=1:7
          zp(1)=zp(1)+nk*randi(20);
          zp(2)=zp(2)+nk*randi(20);
          zp(3)=zp(3)+nk*randi(10);
          poly1=[poly1',zp']';
         end
        %v1=(4*pi*(avg_min_dist1/2)^3)/3;
        %v1=numel(e2)*v1;
        %vol1=v1;
        [C1,v1] = convhulln(poly1);
        poly11=poly1(C1,:);
        vol1=v1;
   end
...
ps=PU;
  ...
  if isempty(ps)==0 && size(ps,1)>3
  disp('INTERSECTION STANDARD CLUSTERING')
      pp=intersectionHull('vert',poly2,'vert',poly1);
      P=pp.vert;
      [C3,v3] = convhulln(P);
      stat=1;
           ...
           colocatv(3,1)=v2;
           colocatv(4,1)=v3;
           colocatv(5,1)= (v3/v2)*100;
           colocatv(6,1)= v1;
           colocat=[colocat,colocatv];
       figure(200)
       subplot('position',pozycja1)
        plot3(ps(:,1),ps(:,2),ps(:,3),'ow','markerfacecolor','r','markersize',6)
        hold on
        trisurf(C3,P(:,1),P(:,2),P(:,3),'FaceColor','w','FaceAlpha',0.8)
        
  elseif isempty(ps)==0 && size(ps,1)<=3    
          disp('INTERSECTION STANDARD SMALL')
          %v3=(4*pi*(avg_min_dist2/2)^3)/3;
          %v3=size(ps,1)*v3;
          pp=intersectionHull('vert',poly2,'vert',poly1);
           P=pp.vert;
           [C3,v3] = convhulln(P);
