...
cid2centr(kk,8)=et;
cid2centr(kk,9)=poa1;
cid2centr(kk,10)=numel(eoo);
cid2centr(kk,11)=numel(ett);
...
disp(['intersection search ',num2str(j),'/',num2str(size(Cl,1))])
 ...
 %%%%%%%%%%%%%%%%%%%%%intersection %%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    hold on
    %trisurf(C2,hull2(:,1),hull2(:,2),hull2(:,3),'FaceColor','r','FaceAlpha',0.4)
    plot3(hull1(:,1),hull1(:,2),hull1(:,3),'.w','markersize',6)
...
coloc=coloc';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%filter 1-point cluster

ef=find(cid2centr(:,10)==1);
cid2centr(ef,:)=[];
