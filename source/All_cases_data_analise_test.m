...
hold on
 [ke,xx]=ksdensity(d_nint,px1,'kernel','epanechnikov');
  ...
  kenint(1:e1-1)=0;
  plot(px,kenint(1:end),'-k','linewidth',2)
 ...
 [ke,xx]=ksdensity(d_int,px,'kernel','epanechnikov');
  ...
  %ke2=ke2/sum(ke2);
  keint=ke;
  plot(px,keint(1:end),'-b','linewidth',2)
 ...
 % xlim([-200,max(px)])
  xlim([-100,500])
 title(['Histogram and empirical PDF distance distributions Radius ',num2str(RRR),...
     '  nm \newline Sample1 ',num2str(exname1),' Sample2 ',num2str(exname2)])
grid on
...
grid on
 ...
 xlim([-100,500])
  grid on
  title(['Histogram of distance distributions. Radius ',num2str(RRR),...
...
grid on
 ...
 for i=1:size(px,2)
  ...
  legend('Empirical PDF for non-collocalized cluster','Empirical PDF for collocalized cluster','Possible misclassification of collocation')
   title(['Empirical PDF distance distributions Radius  ',num2str(RRR),...
